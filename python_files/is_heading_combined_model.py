...
df_c = pd.read_csv(r'E:\Cash App\LITM\New Accounts\Unilever\CSVs\COMBINED_CSV.csv',
                      encoding='cp1256')
...
def func(df_c):
    ...
    for i in range(0, df_c.shape[0]):
        ...
        if (re.search('[a-z0-9]*((date[a-z0-9]*(\s)?inv))[a-z0-9]*',
                          df_c['row_string_new'].values[i]) == None):
...
label=df_c['is_heading']
train_features,test_features,train_labels,test_labels=train_test_split(df_c,label,test_size=0.2,random_state=42)
#####################################################
#train data
#####################################################

train_features=clean_data(train_features)
train_features=func(train_features)
train_features['alpha_numeric_ratio']=train_features['row_string_new'].apply(alpha_func)
train_features['distance_from_top']=train_features['row_distanceFromTop'].apply(distance_from_top)
...
test_features=func(test_features)
test_features['alpha_numeric_ratio']=test_features['row_string_new'].apply(alpha_func)
test_features['distance_from_top']=test_features['row_distanceFromTop'].apply(distance_from_top)
...
rf.fit(train_features[vocab],train_labels)
pred=rf.predict(test_features_combined[vocab])
...
#####################################################
x=pd.DataFrame()
x['actual']=test_labels_combined['is_heading']
x['pred']=pred
data_temp=pd.DataFrame()
data_temp=test_features_combined[vocab]
data_temp['actual']=x['actual']
data_temp['pred']=x['pred']
data_temp['row_string_new']=test_features_combined['row_string_new']
data_temp['check_number']=test_features_combined['check_checkNumber']
path='C:\\Users\\gaurav.subedar\\Desktop\\doc_classification_Unilever'
data_temp.to_csv(os.path.join(path,r'test_data_combined_misclassifications.csv'))
...
print(train_features[vocab].columns)
for f in range(train_features[vocab].shape[1]):
    print("%d. feature %d (%f)" % (f + 1, indices[f], importances[indices[f]]),vocab[indices[f]])
